package com.huindata.integration.whhealth.covid19.vo;

import com.fasterxml.jackson.annotation.JsonAutoDetect;
import com.fasterxml.jackson.annotation.JsonProperty;

/**
 *
 */

@JsonAutoDetect(fieldVisibility= JsonAutoDetect.Visibility.ANY, getterVisibility=JsonAutoDetect.Visibility.NONE)
public class CYXXRequestBody implements RequestBody{
    @JsonProperty("YLJGDM")
    private String YLJGDM;

    @JsonProperty("SGTXM")
    private String SGTXM;

    @JsonProperty("BBLX")
    private String BBLX;

    @JsonProperty("YLJGMC")
    private String YLJGMC;

    @JsonProperty("JCJGDM")
    private String JCJGDM;

    @JsonProperty("JCJGMC")
    private String JCJGMC;

    @JsonProperty("CYFZR")
    private String CYFZR;

    @JsonProperty("LXFS")
    private String LXFS;

    @JsonProperty("CYDD")
    private String CYDD;

    @JsonProperty("CYRQ")
    private String CYRQ;

    @JsonProperty("HYFS")
    private String HYFS;

    @JsonProperty("SGLYDW")
    private String SGLYDW;

    @JsonProperty("SSRQ")
    private String SSRQ;

    @JsonProperty("RYLX")
    private String RYLX;

    @JsonProperty("BLLX")
    private String BLLX;

    @JsonProperty("PID")
    private String PID;

    @JsonProperty("YWHDLX")
    private String YWHDLX;

    @JsonProperty("JZLSH")
    private String JZLSH;

    @JsonProperty("KH")
    private String KH;

    @JsonProperty("KLX")
    private String KLX;

    @JsonProperty("JJCS")
    private String JJCS;

    @JsonProperty("ZJHM")
    private String ZJHM;

    @JsonProperty("ZJLX")
    private String ZJLX;

    @JsonProperty("XM")
    private String XM;

    @JsonProperty("CSRQ")
    private String CSRQ;

    @JsonProperty("XB")
    private String XB;

    @JsonProperty("MZ")
    private String MZ;

    @JsonProperty("LXDH")
    private String LXDH;

    @JsonProperty("LXRXM")
    private String LXRXM;

    @JsonProperty("LXRZJHM")
    private String LXRZJHM;

    @JsonProperty("LXRZJLX")
    private String LXRZJLX;

    @JsonProperty("GXDM")
    private String GXDM;

    @JsonProperty("LXRLXDH")
    private String LXRLXDH;

    @JsonProperty("BCYRDZ_MPH")
    private String BCYRDZ_MPH;

    @JsonProperty("BCYRDZ_XXMS")
    private String BCYRDZ_XXMS;

    public String getYLJGDM() {
        return YLJGDM;
    }

    public void setYLJGDM(String YLJGDM) {
        this.YLJGDM = YLJGDM;
    }

    public String getSGTXM() {
        return SGTXM;
    }

    public void setSGTXM(String SGTXM) {
        this.SGTXM = SGTXM;
    }

    public String getBBLX() {
        return BBLX;
    }

    public void setBBLX(String BBLX) {
        this.BBLX = BBLX;
    }

    public String getYLJGMC() {
        return YLJGMC;
    }

    public void setYLJGMC(String YLJGMC) {
        this.YLJGMC = YLJGMC;
    }

    public String getJCJGDM() {
        return JCJGDM;
    }

    public void setJCJGDM(String JCJGDM) {
        this.JCJGDM = JCJGDM;
    }

    public String getJCJGMC() {
        return JCJGMC;
    }

    public void setJCJGMC(String JCJGMC) {
        this.JCJGMC = JCJGMC;
    }

    public String getCYFZR() {
        return CYFZR;
    }

    public void setCYFZR(String CYFZR) {
        this.CYFZR = CYFZR;
    }

    public String getLXFS() {
        return LXFS;
    }

    public void setLXFS(String LXFS) {
        this.LXFS = LXFS;
    }

    public String getCYDD() {
        return CYDD;
    }

    public void setCYDD(String CYDD) {
        this.CYDD = CYDD;
    }

    public String getCYRQ() {
        return CYRQ;
    }

    public void setCYRQ(String CYRQ) {
        this.CYRQ = CYRQ;
    }

    public String getHYFS() {
        return HYFS;
    }

    public void setHYFS(String HYFS) {
        this.HYFS = HYFS;
    }

    public String getSGLYDW() {
        return SGLYDW;
    }

    public void setSGLYDW(String SGLYDW) {
        this.SGLYDW = SGLYDW;
    }

    public String getSSRQ() {
        return SSRQ;
    }

    public void setSSRQ(String SSRQ) {
        this.SSRQ = SSRQ;
    }

    public String getRYLX() {
        return RYLX;
    }

    public void setRYLX(String RYLX) {
        this.RYLX = RYLX;
    }

    public String getBLLX() {
        return BLLX;
    }

    public void setBLLX(String BLLX) {
        this.BLLX = BLLX;
    }

    public String getPID() {
        return PID;
    }

    public void setPID(String PID) {
        this.PID = PID;
    }

    public String getYWHDLX() {
        return YWHDLX;
    }

    public void setYWHDLX(String YWHDLX) {
        this.YWHDLX = YWHDLX;
    }

    public String getJZLSH() {
        return JZLSH;
    }

    public void setJZLSH(String JZLSH) {
        this.JZLSH = JZLSH;
    }

    public String getKH() {
        return KH;
    }

    public void setKH(String KH) {
        this.KH = KH;
    }

    public String getKLX() {
        return KLX;
    }

    public void setKLX(String KLX) {
        this.KLX = KLX;
    }

    public String getJJCS() {
        return JJCS;
    }

    public void setJJCS(String JJCS) {
        this.JJCS = JJCS;
    }

    public String getZJHM() {
        return ZJHM;
    }

    public void setZJHM(String ZJHM) {
        this.ZJHM = ZJHM;
    }

    public String getZJLX() {
        return ZJLX;
    }

    public void setZJLX(String ZJLX) {
        this.ZJLX = ZJLX;
    }

    public String getXM() {
        return XM;
    }

    public void setXM(String XM) {
        this.XM = XM;
    }

    public String getCSRQ() {
        return CSRQ;
    }

    public void setCSRQ(String CSRQ) {
        this.CSRQ = CSRQ;
    }

    public String getXB() {
        return XB;
    }

    public void setXB(String XB) {
        this.XB = XB;
    }

    public String getMZ() {
        return MZ;
    }

    public void setMZ(String MZ) {
        this.MZ = MZ;
    }

    public String getLXDH() {
        return LXDH;
    }

    public void setLXDH(String LXDH) {
        this.LXDH = LXDH;
    }

    public String getLXRXM() {
        return LXRXM;
    }

    public void setLXRXM(String LXRXM) {
        this.LXRXM = LXRXM;
    }

    public String getLXRZJHM() {
        return LXRZJHM;
    }

    public void setLXRZJHM(String LXRZJHM) {
        this.LXRZJHM = LXRZJHM;
    }

    public String getLXRZJLX() {
        return LXRZJLX;
    }

    public void setLXRZJLX(String LXRZJLX) {
        this.LXRZJLX = LXRZJLX;
    }

    public String getGXDM() {
        return GXDM;
    }

    public void setGXDM(String GXDM) {
        this.GXDM = GXDM;
    }

    public String getLXRLXDH() {
        return LXRLXDH;
    }

    public void setLXRLXDH(String LXRLXDH) {
        this.LXRLXDH = LXRLXDH;
    }

    public String getBCYRDZ_MPH() {
        return BCYRDZ_MPH;
    }

    public void setBCYRDZ_MPH(String BCYRDZ_MPH) {
        this.BCYRDZ_MPH = BCYRDZ_MPH;
    }

    public String getBCYRDZ_XXMS() {
        return BCYRDZ_XXMS;
    }

    public void setBCYRDZ_XXMS(String BCYRDZ_XXMS) {
        this.BCYRDZ_XXMS = BCYRDZ_XXMS;
    }
}
